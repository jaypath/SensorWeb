...
constexpr uint8_t UDP_MSG_PING_RESPONSE_REQUIRED  = 128; // Private: ping request response, payload = none
constexpr uint8_t UDP_MSG_PING_RESPONSE_SUCCESS = 129;   // Private: ping response received
...
constexpr uint8_t UDP_MSG_BROADCAST_ALIVE      = 0;   // Broadcast: device is alive (all devices, periodic)
constexpr uint8_t UDP_MSG_SERVER_LIST          = 1;   // Private: payload contains up to 6 server addresses (MAC+IP)
...
bool sendESPNOW(ESPNOW_type msg, bool forceencrypt = false);
bool sendESPNowUDPMessage(ESPNOW_type msg, IPAddress targetIP, bool forceencrypt = false);
