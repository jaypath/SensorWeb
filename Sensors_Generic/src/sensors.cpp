...
#ifdef _WEBCHART
  SensorChart SensorCharts[SENSORS_TO_CHART];
#endif
...
uint  sc_interval; 
  ...
  for (byte i=0;i<SENSORNUM;i++) {
    ...
    switch (SENSORTYPES[i]) {
      ...
      case 2: //DHT RH
        ...
        break;
      case 3: //soil
        sc_multiplier = 0.01; //divide by 100
        sc_offset=0;
        sc_interval=60*30;//seconds 
        #ifdef _USESOILCAP
  ...
  for (byte j=0;j<_WEBCHART;j++) {
    SensorCharts[j].sensornum = find_sensor_type(SENSORS_TO_CHART[j],255);
    SensorCharts[j].offset = sc_offset;
    SensorCharts[j].multiplier = sc_multiplier
    for (byte i;i<50;i++)     SensorCharts[j].values[i] = 0;
