...
void initPeripheralSensors() {
  ...
  //update MY_DEVICE_INDEX

  //update mySensorsIndices array
  MY_DEVICE_INDEX = Sensors.findMyDeviceIndex(); //update this periodically to ensure I have no changed
  if (MY_DEVICE_INDEX == -1) {
    //register myself as a device
...
//int sc_offset;
  ...
  for (byte i=0;i<SENSORNUM;i++) {
    ...
    switch (stype) {
      ...
      {
        #ifdef DHTTYPE
          SensorHistory.SensorIDs[i] = MY_DEVICE_INDEX<<16 + stype<<8 + Sensors.countSensors(stype); //Sensors.countSensors(stype) returns a 1-based index, so no need to subtract 1
          
          SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), stype, 0, String(MYNAME+String("_T")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
        #endif
        break;
      }
      case 2: //DHT RH
       {
        ...
        #ifdef DHTTYPE
          SensorHistory.SensorIDs[i] = MY_DEVICE_INDEX<<16 + stype<<8 + Sensors.countSensors(stype); //Sensors.countSensors(stype) returns a 1-based index, so no need to subtract 1
           SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), stype, 0, String(MYNAME+String("_RH")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
        #endif
        break;
       }
      case 3: //soil
        ...
        #ifdef _USESOILCAP
          SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), stype, 0, String(MYNAME+String("_soil")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
        #endif
        #ifdef _USESOILRES
          pinMode(_USESOILRES,OUTPUT);  
          SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), stype, 0, String(MYNAME+String("_soilR")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
        #endif
      ...
      case 4: //AHT temp
        {
          ...
          SensorHistory.SensorIDs[i] = MY_DEVICE_INDEX<<16 + stype<<8 + Sensors.countSensors(stype); //Sensors.countSensors(stype) returns a 1-based index, so no need to subtract 1
          SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), 4, 0, String(MYNAME+String("_AHT_T")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
        #endif
        break;
        }
      case 5: //aht rh
        ...
        SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), 5, 0, String(MYNAME+String("_AHT_RH")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
        #endif
        break;
        }
      ...
      case 7: //dist
        ...
        bitWrite(flags,7,0);
        SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), 7, 0, String(MYNAME+String("_Dist")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
        break;
        }
      case 9: //BMP pres
        {
        ...
        SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), 9, 0, String(MYNAME+String("_hPa")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
        break;
        }
      case 10: //BMP temp
        {
        ...
        SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), 10, 0, String(MYNAME+String("_BMP_t")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
        break;
        }
      case 11: //BMP alt
        {
        ...
        SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), 11, 0, String(MYNAME+String("_alt")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
        break;
        }
      case 12: //Bar prediction
        ...
        SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), 12, 0, String(MYNAME+String("_Pred")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
        break;
        }
      case 13: //BME pres
        {
        ...
        SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), 13, 0, String(MYNAME+String("_hPa")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
        break;
        }
      case 14: //BMEtemp
        {
        ...
        SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), 14, 0, String(MYNAME+String("_BMEt")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
        break;
        }
      case 15: //bme rh
        {
        ...
        bitWrite(flags,7,0);
        SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), 15, 0, String(MYNAME+String("_BMErh")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
        break;
        }
      case 16: //bme alt
        {
          ...
          bitWrite(flags,7,0);
          SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), 16, 0, String(MYNAME+String("_alt")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
        break;
        }
      case 17: //bme680
        {
        ...
        SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), 17, 0, String(MYNAME+String("_T")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
        break;
        }
      case 18: //bme680
        {
        SensorHistory.SensorIDs[i] = MY_DEVICE_INDEX<<16 + stype<<8 + Sensors.countSensors(stype); //Sensors.countSensors(stype) returns a 1-based index, so no need to subtract 1
        SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), 18, 0, String(MYNAME+String("_RH")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
        break;
        }
      case 19: //bme680
        {
        SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), 19, 0, String(MYNAME+String("_hPa")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
        SensorHistory.SensorIDs[i] = MY_DEVICE_INDEX<<16 + stype<<8 + Sensors.countSensors(stype); //Sensors.countSensors(stype) returns a 1-based index, so no need to subtract 1
      ...
      case 20: //bme680
        {
        SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), 20, 0, String(MYNAME+String("_VOC")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
        SensorHistory.SensorIDs[i] = MY_DEVICE_INDEX<<16 + stype<<8 + Sensors.countSensors(stype); //Sensors.countSensors(stype) returns a 1-based index, so no need to subtract 1
        ...
        case 53: //HVAC time - this is the total time. Note that sensor pin is not used
         {
          ...
          SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), 50, 0, String(MYNAME+String("_Total")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
        ...
        case 54: //ac, gas valve
        {
          ...
          SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), 51, 0, String(MYNAME+String("_GAS")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
          SensorHistory.SensorIDs[i] = MY_DEVICE_INDEX<<16 + stype<<8 + Sensors.countSensors(stype); //Sensors.countSensors(stype) returns a 1-based index, so no need to subtract 1
        ...
        case 55: //heat
        {
          ...
          int16_t HZindex = Sensors.countSensors(55,MY_DEVICE_INDEX); //returns a 1-based index. this will be the snsID

          SensorHistory.SensorIDs[i] = MY_DEVICE_INDEX<<16 + stype<<8 + Sensors.countSensors(stype); //Sensors.countSensors(stype) returns a 1-based index, so no need to subtract 1

          String heatName = String(MYNAME) + "_" + String(HEATZONE[HZindex]); //no HZindex-1 because we are using a 1-based index
          SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), 55, HZindex, heatName.c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
          #ifdef _USEMUX
          #else
        ...
        case 56: //aircon compressor
          {
            SensorHistory.SensorIDs[i] = MY_DEVICE_INDEX<<16 + stype<<8 + Sensors.countSensors(stype); //Sensors.countSensors(stype) returns a 1-based index, so no need to subtract 1
            SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), 56, 0, String(MYNAME+String("_comp")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
          break;
          }
        case 57: //aircon fan
          {
          SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), 57, 0, String(MYNAME+String("_fan")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
          SensorHistory.SensorIDs[i] = MY_DEVICE_INDEX<<16 + stype<<8 + Sensors.countSensors(stype); //Sensors.countSensors(stype) returns a 1-based index, so no need to subtract 1
      ...
      case 70: //leak
        {
          #ifdef _USELEAK
          SensorHistory.SensorIDs[i] = MY_DEVICE_INDEX<<16 + stype<<8 + Sensors.countSensors(stype); //Sensors.countSensors(stype) returns a 1-based index, so no need to subtract 1
          SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), 70, 0, String(MYNAME+String("_leak")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
          #endif
          break;
        }
      ...
      case 60: //battery
        {
          bitWrite(flags,1,0);
          SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), 60, 0, String(MYNAME+String("_bat")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
          SensorHistory.SensorIDs[i] = MY_DEVICE_INDEX<<16 + stype<<8 + Sensors.countSensors(stype); //Sensors.countSensors(stype) returns a 1-based index, so no need to subtract 1
      ...
      case 61: //battery percent
        {
          ...
          bitWrite(flags,3,1);
          SensorHistory.SensorIDs[i] = MY_DEVICE_INDEX<<16 + stype<<8 + Sensors.countSensors(stype); //Sensors.countSensors(stype) returns a 1-based index, so no need to subtract 1
          SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), 61, 0, String(MYNAME+String("_bpct")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);
        break;
        }
      case 90: //Sleep info
        {
          ...
          SensorHistory.SensorIDs[i] = MY_DEVICE_INDEX<<16 + stype<<8 + Sensors.countSensors(stype); //Sensors.countSensors(stype) returns a 1-based index, so no need to subtract 1
          SensorHistory.sensorIndex[i] = Sensors.addSensor(ESP.getEfuseMac(), WiFi.localIP(), 90, 0, String(MYNAME+String("_sleep")).c_str(), 0, 0, 0, Prefs.SNS_INTERVAL_SEND[i], flags, String(MYNAME).c_str(), MYTYPE);          
        break;
        }
...
int8_t ReadData(struct SnsType *P, bool forceRead, int16_t mydeviceindex) {
  //return -2 if I am not registered, -1 if not my sensor, 0 if not time to read, 1 if read successful
