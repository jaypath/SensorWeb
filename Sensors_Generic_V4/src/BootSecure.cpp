...
int8_t BootSecure::getPrefs() {
    ...
    if (p.isKey("Boot")) {
        uint8_t padding = 0;
        if (sizeof(STRUCT_PrefsH) % 16 != 0) padding = 16 - (sizeof(STRUCT_PrefsH) % 16);
...
int8_t BootSecure::encrypt(const unsigned char* input, uint16_t inputlength, char* key, unsigned char* output, uint16_t* outputlength) {
    ...
    if (inputlength == 0 || inputlength % 16 != 0) paddedLen += 16 - (inputlength % 16);
...
// --- Global-style forwarders to keep API parity ---
uint16_t CRCCalculator(uint8_t * data, uint16_t length) { return BootSecure::CRCCalculator(data, length); }
int8_t encrypt(const unsigned char* input, uint16_t inputlength, char* key, unsigned char* output,  uint16_t* outputlength) { return BootSecure::encrypt(input, inputlength, key, output, outputlength); }
int8_t decrypt(unsigned char* input, char* key, unsigned char* output, uint16_t datalength) { return BootSecure::decrypt(input, key, output, datalength); }
...
// Provide a non-static wrapper for setPrefs parity
bool setPrefs() { return BootSecure::setPrefs(); }
