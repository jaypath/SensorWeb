...
bool file_uploadData(byte SnsIndex, String fileID);
...
void initSensor(byte k) {
  ...
  Sensors[k].snsValue_MAX=-1000;
  Sensors[k].snsValue_MIN=-1000;
...
void setup()
{
 ...
 tft.init();
  ...
  tft.printf("FLASH size: %d bytes",ESP.getFlashChipSize());
...
//just delete all spreadsheets!


file_deleteSpreadsheetByName("ArduinoLog2024-02");
file_deleteSpreadsheetByName("ArduinoLog2106-02");
file_deleteSpreadsheetByName("ArduinoLog106-02");
file_deleteSpreadsheetByName("jaywashere2");

tft.printf("%s\n",file_findSpreadsheetIDByName("***"));
...
void drawScreen() {
  ...
  sprintf(tempbuf,"%s",dateify(now(),"hh:nn"));
  Ypos = Ypos + tft.fontHeight(8)/2;
  tft.drawString(tempbuf, tft.width() / 2, Ypos);

  Ypos = Ypos + tft.fontHeight(8)/2 + 5;
  ...
  for(byte i=0;i<SENSORNUM;i++) {
    ...
    if (Sensors[i].snsID>0 && Sensors[i].snsType>0 && inIndex(i,used,SENSORNUM) == false && Sensors[i].timeLogged>0 ) {
      used[usedINDEX++] = i;
      fcnChooseTxtColor(i);
      tft.printf("%d %s: [%d < %d < %d] %s %s",Sensors[i].ardID, Sensors[i].snsName, (int) Sensors[i].snsValue_MIN, (int) Sensors[i].snsValue,Sensors[i].snsValue_MAX,dateify(Sensors[i].timeLogged,"hh:nn mm/dd"), Sensors[i].isSent ? "SENT" : "");
    ...
    // organize by sensor
      for(byte j=i+1;j<SENSORNUM;j++) {
        if (Sensors[j].snsID>0 && Sensors[j].snsType>0 && inIndex(j,used,SENSORNUM) == false && Sensors[j].ardID==Sensors[i].ardID) {
          used[usedINDEX++] = j;
          fcnChooseTxtColor(j);
          tft.printf("%d %s: %d %s %s",Sensors[j].ardID, Sensors[j].snsName, (int) Sensors[j].snsValue,dateify(Sensors[j].timeLogged,"hh:nn mm/dd"), Sensors[i].isSent ? "SENT" : "");
        }
      }
    }
  tft.setTextColor(FG_COLOR,BG_COLOR);
...
String file_findSpreadsheetIDByName(String sheetname, bool createfile) {
 ...
 tft.printf("Trying to find %s, result: %s\n",sheetname.c_str(),success?"OK":"FAIL");
...
//          fileinfo.toString(tmp,true);
    ...
    if (createfile) {
      ...
      tft.printf("new fileid: %s\n",resultstring.c_str());
    }         
       
    
  } else { //gsheet error
    tft.printf("Gsheet error... \n");
...
if (fileID.substring(0,5)!="ERROR") return fileID;
  ...
  if (success) {
    ...
    return file_findSpreadsheetIDByName(sheetname);
    
  
  } 
  
   tft.printf("CREATE: Error with gsheet to create object\n");
...
bool file_uploadData(byte SnsIndex, String fileID) {
  ...
  String logid;
  logid = (String) Sensors[SnsIndex].ardID + "." + (String)  Sensors[SnsIndex].snsType + "." + (String)  Sensors[SnsIndex].snsID;



  valueRange.add("majorDimension", "ROWS");
  valueRange.set("values/[0]/[0]", logid);
  valueRange.set("values/[0]/[1]", Sensors[SnsIndex].IP.toString());
  valueRange.set("values/[0]/[2]", (String) Sensors[SnsIndex].snsName);
  valueRange.set("values/[0]/[3]", (String) Sensors[SnsIndex].timeLogged);
  valueRange.set("values/[0]/[4]", (String) Sensors[SnsIndex].timeRead);
  valueRange.set("values/[0]/[5]", (String) dateify(Sensors[SnsIndex].timeRead,"mm/dd/yy hh:nn:ss"));
  valueRange.set("values/[0]/[6]", (String) bitRead(Sensors[SnsIndex].Flags,0));
  valueRange.set("values/[0]/[7]", (String) Sensors[SnsIndex].snsValue);
...
//            response.toString(Serial, true);
          Sensors[SnsIndex].isSent = true;
        else
          Sensors[SnsIndex].isSent = false;
...
uint8_t tempIP[4] = {0,0,0,0};
  ...
  for (byte k=0;k<server.args();k++) {
      ...
      if ((String)server.argName(k) == (String)"varName") sprintf(S.snsName,"%s", server.arg(k).c_str());
